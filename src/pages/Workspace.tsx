...
const Workspace = () => {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      // When the tab becomes hidden, immediately trigger a save
      if (document.visibilityState === 'hidden') {
        saveCurrentChat();
      }
    };

    // Listen for tab switches, minimizing, and mobile app backgrounding
    document.addEventListener('visibilitychange', handleVisibilityChange);

    // Cleanup: Also save if the component unmounts (e.g., navigating away via React Router)
    return () => {
    ...
    <div className="h-dvh bg-slate-50 flex overflow-hidden">
      ...
      `}>
        ...
        {!isCollapsed || isMobileMenuOpen ? (
          ...
          <div className="mb-4 mt-4 flex justify-center hover:bg-slate-50 p-2 rounded" onClick={()=>setIsCollapsed(false)}>
